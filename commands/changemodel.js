...
module.exports = {
	...
	execute: async function(msg, context, args) {
        ...
        if (args.length == 1) {
            ...
            console.log(2)
            const request2 = await axios.post(`${url}/sdapi/v1/options`, { sd_model_checkpoint: data[modelMatchIndex - 2].title})
            return `Changed model to ${data[modelMatchIndex - 2].model_name}`
        }
		return "dejj"
	}
}