...
export async function getCommandFromAlias(alias, channel_id) {
	let data = await queryDatabase("SELECT command FROM aliases WHERE alias = ? AND channel_id = ? LIMIT 1", [alias, channel_id])
