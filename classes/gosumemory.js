...
var fs = require('fs');
const liveppCalcDirectory = config.osu.PerformanceCalculator_Live_folder
const reworkppCalcDirectory = config.osu.PerformanceCalculator_Rework_folder
...
class GosuMemory {
    ...
    async getPPCustom(accuracy) {
        var beatmap = parser.parseContent(fs.readFileSync(this.getOsuFile()))
        ...
        var commandString = `${liveppCalcDirectory} simulate osu "${this.getOsuFile()}" -G ${numberOf100sNeeded} ${this.getModsForPPCalc()} --json`
    ...
    getOsuFile() {
        return `${songsFolder}${this.menu.bm.path.folder}\\${this.menu.bm.path.file}`
    }
    getModsForPPCalc() {
    ...
    async getCurrentPP() {
        var commandString = `${liveppCalcDirectory} simulate osu "${this.getOsuFile()}" -G ${this.gameplay.hits["100"]} -M ${this.gameplay.hits["50"]} -X ${this.gameplay.hits["0"]} -c ${this.gameplay.combo.max} ${this.getModsForPPCalc()} --json`
        var currentPP = await calculatePP(commandString)
        return currentPP
    }
    async getPPIfFc() {
        var adjusted100sCount = this.gameplay.hits["100"] + this.gameplay.hits["0"]
        var commandString = `${liveppCalcDirectory} simulate osu "${this.getOsuFile()}" -G ${adjusted100sCount} -M ${this.gameplay.hits["50"]} -X 0 -c ${this.getMaxCombo()} ${this.getModsForPPCalc()} --json`
        var currentPP = await calculatePP(commandString)
        return currentPP
    }
    async getSSPP() {
        var commandString = `${liveppCalcDirectory} simulate osu "${this.getOsuFile()}" ${this.getModsForPPCalc()} --json`
        var currentPP = await calculatePP(commandString)
        return currentPP
    }
    async getReworkPP() {
        var commandString = `${reworkppCalcDirectory} simulate osu "${this.getOsuFile()}" -G ${this.gameplay.hits["100"]} -M ${this.gameplay.hits["50"]} -X ${this.gameplay.hits["0"]} -c ${this.gameplay.combo.max} ${this.getModsForPPCalc()} --json`
        var currentPP = await calculatePP(commandString)
        return currentPP
    }
    getSR() {
...
async function calculatePP(calcString, isSr) {
    return new Promise(function(resolve, reject) {
        exec(calcString, { windowsHide: true }, async function(err, stdout, stderr) {
            try {
